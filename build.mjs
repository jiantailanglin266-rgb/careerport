...
function prerender(p) {
  ...
  if (p.kind === "home") {
    ...
    body += `<h2>統計データ</h2><p>出典名・出典URL・データ時点・更新日を明示できるもののみ掲載し、未接続の項目は「データ準備中」と表示します。</p><h2>テキスト・画像</h2><p>Wikipedia引用は CC BY-SA 4.0 に基づき出典を明記します。引用画像は Wikipedia / Wikimedia Commons のライセンス表記可能なもののみ使用し、作者・ライセンスを明記します。メインビジュアル等の装飾画像には権利処理済みの自社制作ビジュアル（AI生成を含む）を使用する場合があります。</p><h2>求人情報</h2><p>正規のデータ提携によるもののみ掲載し、無断転載・スクレイピングによる複製は行いません。</p>`;
